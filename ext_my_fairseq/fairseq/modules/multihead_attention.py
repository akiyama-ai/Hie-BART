...
class MultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        if key_padding_mask is not None:
        #if key_padding_mask is not None and not hie_self_attn:  # For Hie Con
        ...
        # For Hie
        heads_attn = attn.clone()
        
        if hie_self_attn:
            adj_head = self.half_num_heads + self.join_head
            attn = torch.cat([self.heads_attn[:bsz * adj_head, :, :], heads_attn[bsz * adj_head:, :, :]], dim=0)  # word_sent adj
