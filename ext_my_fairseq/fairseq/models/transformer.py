...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        # For Hie
        parser.add_argument('--encoder-attention-join-head', type=int,
                            help='num encoder attention join head')
        # fmt: on
...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        #self.segment_embed = Embedding(2, embed_dim, self.padding_idx)  # Foe Hie
        self.device = torch.device('cuda')
        
        #self.embed_segments = SinusoidalPositionalEmbedding.get_embedding(4, embed_dim, self.padding_idx).to(device)
        self.embed_segments = SinusoidalPositionalEmbedding.get_embedding(402, embed_dim, self.padding_idx).to(self.device).half()
        #self.embed_segments_ = TransformerSentenceEncoder(padding_idx = self.padding_idx, vocab_size = len(dictionary))
        #self.emb = nn.Embedding(2, embed_dim, self.padding_idx)
        #self.emb.weight.requires_grad = False
        
        #self.embed_segments = SinusoidalPositionalEmbedding.get_embedding(3, embed_dim)
        #self._float_tensor = SinusoidalPositionalEmbedding.register_bufferx
    ...
    def segment_sent(self, tokens):    # For Hie
        ...
        segment_list = []
        #zero_segments = self.embed_segments[2].clone()
        #one_segments = self.embed_segments[3].clone()
        ...
        for i in range(segment_flag.shape[0]):
            ...
            for j in range(segment_flag.shape[1]):    # Every Batch
                ...
                if segment_sent[i][j] == self.padding_idx:
                    #segment_sent[i][j] = 0.0
                    segments_mini_list.append(zero_segments)
                elif cnt % 2 == 0:
                    #segment_sent[i][j] = 0.0
                    segments_mini_list.append(zero_segments)
                    #segments_mini_list.append(self.embed_segments_.segment_embeddings(0))
                elif cnt % 2 != 0:
                    #segment_sent[i][j] = 1.0
                    segments_mini_list.append(one_segments)
                    #segments_mini_list.append(self.embed_segments_.segment_embeddings(1))
                    
            #segments = torch.stack(segments_mini_list, dim=0)
            segments = torch.cat(segments_mini_list)
            segment_list.append(segments)
            """
            for j in range(segment_flag.shape[1]):    # Every Batch PE
                if segment_flag[i][j] == True:
                    cnt += 1
                ...
                if segment_sent[i][j] == self.padding_idx:
                    segments_mini_list.append(self.embed_segments[self.padding_idx])
                    #segments_mini_list.append(self.padding_idx)
                else:
                    segments_mini_list.append(self.embed_segments[cnt+2])
                    #segments_mini_list.append(cnt+2)
            segments = torch.stack(segments_mini_list, dim=0)
            segment_list.append(segments)
            ##segment_list.append(segments_mini_list)
            """
        segments_emb = torch.stack(segment_list, dim=0)
        ##segment_sent = segment_list
        
        #emb = self.embed_segments(segment_sent)
        #print(self.embed_segments[0][300])
        #print(self.embed_segments.shape)  # [B, T]        
        #test = torch.where(segment_sent == 0, self.embed_segments[0], self.embed_segments[1])
        
        ##segment_sent = segments_emb
        segment_sent = segments_emb.unsqueeze(-1).repeat(1, 1, self.embed_dim)  # [B, T] -> [B, T, C]
        ##segment_sent = self.embed_positions(segment_sent)
        
        #print(segment_sent[0, :, 0])
        #print(segment_sent.shape)
        
        #print(segments_emb[0, :, 0])
        #print(segments_emb.shape)
        #return segments_emb
        return segment_sent
    """
    def segment_embed(self, tokens):
        emb = tokens
        return emb
    """
    def forward_embedding(self, src_tokens):
        ...
        #if self.segmenting:
        #x = embed + self.segment_sent(src_tokens)  # For Hie
        
        if self.embed_positions is not None:
            x = embed + self.embed_positions(src_tokens)
            
        #x = x + self.segment_sent(src_tokens)  # For Hie
    ...
    def forward_sent_embedding(self, src_tokens):  # For Hie
        # embed tokens and positions
        x = embed = self.embed_scale * self.embed_tokens(src_tokens)
        ...
        x = embed + self.segment_sent(src_tokens)
    ...
    def forward(
        ...
        segments = src_tokens.clone()     # For Hie
        segments = segments.eq(50261)

        x, encoder_embedding = self.forward_embedding(src_tokens)
        #sent_x, sent_encoder_embedding = self.forward_sent_embedding(src_tokens)  # For Hie
        ...
        for layer in self.layers:
            ...
            if not self.training or (dropout_probability > self.encoder_layerdrop):
                ...
                x = layer(x, encoder_padding_mask, segments=segments, src=src_tokens)  #For Hie
                #sent_x = layer(sent_x, encoder_padding_mask)  # For Hie
                #x = (x + sent_x) / 2.0  # For Hie
