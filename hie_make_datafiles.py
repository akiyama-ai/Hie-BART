...
finished_files_dir = "hie_cnn_dm"

# These are the number of .story files we expect there to be in cnn_stories_dir and dm_stories_dir
num_expected_cnn_stories = 92579
num_expected_dm_stories = 219506

#SENTENCE_START = "<s>"  #For hierarchical split
#SENTENCE_END = "</s>"   #For hierarchical split
SENTENCE_START = "<cls>"
SENTENCE_END = "<cls>"
...
def get_art_abs(story_file):
  ...
  article = ' '.join(["%s%s" % (SENTENCE_START, sent) for sent in article_lines])  # For hierarchical split
  #article = ' '.join(["%s%s" % (sent, SENTENCE_END) for sent in article_lines])  # For hierarchical split
